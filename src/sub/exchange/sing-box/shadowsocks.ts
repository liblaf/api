...
export function parseShadowsocks(uri: string): OutboundShadowsocks {
  ...
  if (!matchUri) throw new Error(`Invalid Shadowsocks URI: ${uri}`);
  let { body, tag } = matchUri.groups!;
  body = tryDecodeBase64(body);
  const matchBody = body.match(
    /(?<userinfo>[^@]+)@(?<hostname>[^:]+):(?<port>\d+)/,
  );
  if (!matchBody) throw new Error(`Invalid Shadowsocks URI: ${uri}`);
  ...
  if (!matchUserinfo) throw new Error(`Invalid Shadowsocks URI: ${uri}`);
